                ...
                <div class="breadcrumb-wrapper">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ url()->current() }}">{{ __('Photographers') }}</a>
                        </li>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header border-bottom">
                        ...
                        <a class="btn btn-outline-primary" href="{{ url()->route('photographer.create') }}">
...
@push('script')
    <script>
        var datatable = $('#crudPhotographer').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ url()->route('photographer.data') }}",
            columns: [
                { data: 'no', name: 'no', render: function (data, type, row, meta) {
                        return meta.row + meta.settings._iDisplayStart + 1;
                }, width: '5%', class: 'text-center' },
                { data: 'user.name', name: 'user.name' },
                { data: 'type', name: 'type', width: '5%', class: 'text-center' },
                ...
                { 
                    ...
                    render: function(data, type, row) {
                        var editUrl = "{{ route('photographer.edit', ':id') }}".replace(':id', row.id);
                        var deleteUrl = "{{ route('photographer.destroy', ':id') }}".replace(':id', row.id);
