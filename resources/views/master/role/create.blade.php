                    ...
                    <form action="{{ url()->route('role.store') }}" method="POST">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row form-group mb-1">
                                ...
                                <div class="col-sm-9">
                                    <select id="permission" name="permission[]" class="select2 form-select" multiple data-placeholder=" -- Select Permission --">
                                        <option disabled>{{ __('-- Select Permission --') }}</option>
                                        
                                        @foreach($permissions as $permission)
                                            <option value="{{ $permission->name }}" {{ ( in_array($permission->id, old('permission') ?? []) ? "selected": "") }}>{{ collect(explode(' ', str_replace('-', ' ', $permission->name)))->map(function ($word) { return preg_match('/[aeiou]/i', $word) ? ucwords($word) : strtoupper($word); })->implode(' ') }}</option>
                                        @endforeach
                                    </select>
