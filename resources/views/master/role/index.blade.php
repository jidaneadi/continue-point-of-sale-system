            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header border-bottom">
                        ...
                        <a class="btn btn-outline-primary" href="{{ url()->route('role.create') }}">
    ...
    <script>
        var datatable = $('#crudRole').DataTable({
            ...
            columns: [
                ...
                { data: 'permissions_count', name: 'permissions_count', width: '5%', class: 'text-center' },
                { 
                    data: 'action', 
                    name: 'action', 
                    orderable: false, 
                    ...
                    render: function(data, type, row) {
                        if (row.protected) {
                            return `<span class="badge bg-secondary">{{ __('Protected') }}</span>`;
                        }
                        
                        var editUrl = "{{ route('role.edit', ':id') }}".replace(':id', row.uuid);
                        var deleteUrl = "{{ route('role.destroy', ':id') }}".replace(':id', row.uuid);
